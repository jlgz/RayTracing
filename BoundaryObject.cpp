...
{ 
  ...
  for(int i = 0; i< cares.size();i++){
      Cara c = cares[i];
      vec3 v1 = vec3(this->vertexs[c.idxVertices[0]][0],this->vertexs[c.idxVertices[0]][1],this->vertexs[c.idxVertices[0]][2]);
      vec3 v2 = vec3(this->vertexs[c.idxVertices[1]][0],this->vertexs[c.idxVertices[1]][1],this->vertexs[c.idxVertices[1]][2]);
      vec3 v3 = vec3(this->vertexs[c.idxVertices[2]][0],this->vertexs[c.idxVertices[2]][1],this->vertexs[c.idxVertices[2]][2]);
      Triangle  * ttmp = new Triangle(v1,v2, v3, m);
      this->triangles.push_back(ttmp);
...
bool BoundaryObject::hit(const Ray& r, float t_min, float t_max, HitInfo& rec) const {
    ...
    for(int i = 0; i< triangles.size();i++){
           if(this->triangles[i]->hit( r, t_min, t_max, rec)){
               bf =true;
               if(rec.t< min || min == 0.0f){
                    min = rec.t;
                    infotmp = rec;
               }
           }
...
void BoundaryObject::construeix_cara ( char **words, int nwords, BoundaryObject *objActual, int vindexUlt) {
    ...
    //face.calculaNormal(this->vertexs);
